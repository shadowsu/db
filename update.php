...
<?php
		...
		$nn=$_GET['id'];
		echo $nn;
		$result = mysql_query('SELECT * FROM `'.$table_name.'` WHERE `id` = \'' . $nn . '\'');
						
		while($row = mysql_fetch_array($result)){
			 echo '<form action="update.php?id='.$nn.'" method="get">
		...
		if(isset($_GET['submit'])){
				$sql="UPDATE `".$table_name."` SET `domain`='".$_GET['domain']."',`name`='".$_GET['name']."',`nickname`='".$_GET['nickname']."',`description`='".$_GET['description']."',`ip`='".$_GET['ip']."',`ftpaddress`='".$_GET['ftpaddress']."',`ftpusr`='".$_GET['ftpusr']."',`ftppwd`='".$_GET['ftppwd']."' WHERE `id`=".$nn."";
